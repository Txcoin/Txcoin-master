...
const string strMessageMagic = "Txcoin Signed Message:\n";
...
{
    // Txcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    if ( 0 <= nHeight && nHeight < 20 * 1000)
        return 1000 * COIN;
    else if ( 20 * 1000 <= nHeight && nHeight < 60 * 1000)
        return 500 * COIN;
    else if ( 60 * 1000 <= nHeight && nHeight < 120 * 1000)
        return 100 * COIN;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Txcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Txcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        pchMessageStart[0] = 0xfb;//0xfc+1;
        pchMessageStart[1] = 0xc3;//0xc1+2;
        pchMessageStart[2] = 0xbb;//0xb7+4;
        pchMessageStart[3] = 0xe5;//0xdc+9;
        hashGenesisBlock = uint256("0x00000000798a9bc29886f8e7f34289584f38cfbfcb5e9d79fefbe7150e386b31");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        CPubKey pubkey(ParseHex("028dcef9383ba9cdfa2978937653e9eb73ca13b463a8a56cd06621139139673655"));
        ...
        if (fTestNet)
        {
            block.nTime    = 1401581288;
            block.nNonce   = 1776293887;
        }
        else
        {
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc0, 0xb7, 0xdd }; //{ 0xf9, 0xbe, 0xb4, 0xd9 }; Txcoin: increase each by adding 1,2,3,4 to bitcoin's value.

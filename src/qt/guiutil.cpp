...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("Txcoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("Txcoin://"))
    {
        uri.replace(0, 13, "Txcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Txcoin.lnk";
}
...
{
    return GetAutostartDir() / "Txcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Txcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Txcoin-qt [" + tr("command-line options") + "]                     " + "\n";
